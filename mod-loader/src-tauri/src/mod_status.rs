...
#[command]
pub async fn install_dependencies(mod_id: String, mods_path: String) -> Result<Vec<String>, String> {
    ...
    for dep in status.missing_dependencies {
        ...
        match crate::api_client::get_mod_download_url(dep.modid.clone(), None).await {
            Ok(download_url) => {
                match crate::api_client::download_mod(dep.modid.clone(), download_url, mods_path.clone()).await {
                    Ok(_) => {
                        // Reindex the newly installed mod
                        if let Err(e) = crate::mod_manager::reindex_mod(mods_path.clone(), dep.modid.clone()).await {
                            eprintln!("Failed to reindex {}: {}", dep.modid, e);
                        }
                        installed.push(dep.modid.clone());
                    }
                    Err(e) => {
                        return Err(format!("Failed to download dependency {}: {}", dep.modid, e));
                    }
                }
            }
            Err(e) => {
                return Err(format!("Failed to get download URL for dependency {}: {}", dep.modid, e));
            }
        }
    }
    ...
    for dep in status.outdated_dependencies {
        eprintln!("Updating outdated dependency: {} ({} -> {})", dep.modid, dep.installed, dep.required);
        match crate::api_client::get_mod_download_url(dep.modid.clone(), None).await {
            Ok(download_url) => {
                match crate::api_client::download_mod(dep.modid.clone(), download_url, mods_path.clone()).await {
                    Ok(_) => {
